...
public class CleanNcLogs extends AbstractHandler
{
    public Object execute(ExecutionEvent event) throws ExecutionException
    {
        IPath homepath = UAPEnvHelper.getUAPHomeFolderPath();
        String userhome = homepath + File.separator+"nclogs";
        MDEConsoleFactory.console("BenCheng正在帮你执行清理nclogs操作。。。");
        MDEConsoleFactory.console("nclogs：" + homepath + File.separator + "nclogs" );
        File file = new File(userhome, "nclogs");
        if (!file.exists())
        {
        ...
        {
            CleanChacheJob job = new CleanChacheJob("清理nclogs");
            job.setEvent(event);
